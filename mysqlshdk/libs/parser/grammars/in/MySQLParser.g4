...
alterInstanceStatement:
    {this.serverVersion >= 50713}? INSTANCE_SYMBOL (
        ROTATE_SYMBOL textOrIdentifier MASTER_SYMBOL KEY_SYMBOL
        | {this.serverVersion >= 80016}? RELOAD_SYMBOL TLS_SYMBOL ({this.serverVersion >= 80021}? FOR_SYMBOL CHANNEL_SYMBOL identifier)? (NO_SYMBOL ROLLBACK_SYMBOL ON_SYMBOL ERROR_SYMBOL)?
        | {this.serverVersion >= 80021}? (ENABLE_SYMBOL | DISABLE_SYMBOL) identifier identifier
        | {this.serverVersion >= 80024}? RELOAD_SYMBOL KEYRING_SYMBOL
    )
    | /* empty */
;
...
createProcedure:
    definerClause? PROCEDURE_SYMBOL ({this.serverVersion >= 80029}? ifNotExists)? procedureName OPEN_PAR_SYMBOL (
...
createFunction:
    definerClause? FUNCTION_SYMBOL ({this.serverVersion >= 80029}? ifNotExists)? functionName OPEN_PAR_SYMBOL (
...
createUdf:
    AGGREGATE_SYMBOL? FUNCTION_SYMBOL ({this.serverVersion >= 80029}? ifNotExists)? udfName RETURNS_SYMBOL type = (
        STRING_SYMBOL
        | INT_SYMBOL
        | REAL_SYMBOL
        | DECIMAL_SYMBOL
...
createTrigger:
    definerClause? TRIGGER_SYMBOL ({this.serverVersion >= 80029}? ifNotExists)? triggerName timing = (
        BEFORE_SYMBOL
        | AFTER_SYMBOL
...
queryExpression:
    ({this.serverVersion >= 80000}? withClause)? queryExpressionBody orderClause? limitClause? ({this.serverVersion < 80000}? procedureAnalyseClause)?
;
...
resetOption:
    ...
    | {this.serverVersion < 80000}? QUERY_SYMBOL CACHE_SYMBOL
...
groupReplication:
    START_SYMBOL GROUP_REPLICATION_SYMBOL ({this.serverVersion >= 80021}? groupReplicationStartOptions)?
...
accountManagementStatement:
    ...
    | renameUserStatement
    | revokeStatement
    | {this.serverVersion >= 80000}? setRoleStatement
;

alterUserStatement:
    ALTER_SYMBOL USER_SYMBOL ({this.serverVersion >= 50706}? ifExists)? (
        (
            {this.serverVersion < 80014}? createUserList
            | {this.serverVersion >= 80014}? alterUserList
        ) createUserTail
        | {this.serverVersion >= 80000}? userFunction (
            (identifiedByRandomPassword | identifiedByPassword) ({this.serverVersion >= 80013}? replacePassword)? ({this.serverVersion >= 80014}? retainCurrentPassword)?
            | {this.serverVersion >= 80014}? discardOldPassword
            | {this.serverVersion >= 80027}? userRegistration?
        )
        | {this.serverVersion < 80000}? userFunction identifiedByPassword
        | {this.serverVersion >= 80000}? user (
...
alterUser:
    ...
    | {this.serverVersion >= 80025}? (
        user (
            ...
            | discardOldPassword?
            | {this.serverVersion >= 80027}? ADD_SYMBOL factor identification (ADD_SYMBOL factor identification)?
            | {this.serverVersion >= 80027}? MODIFY_SYMBOL factor identification (
                MODIFY_SYMBOL factor identification
            )?
            | {this.serverVersion >= 80027}? DROP_SYMBOL factor (DROP_SYMBOL factor)?
        )
    )
;
...
oldAlterUser:
    user IDENTIFIED_SYMBOL BY_SYMBOL (
        textString ({this.serverVersion >= 80013}? replacePassword)? ({this.serverVersion >= 80014}? retainCurrentPassword)?
        | {this.serverVersion >= 80018}? RANDOM_SYMBOL PASSWORD_SYMBOL replacePassword? retainCurrentPassword?
    )
    | user IDENTIFIED_SYMBOL WITH_SYMBOL (
        textOrIdentifier (
            BY_SYMBOL textString ({this.serverVersion >= 80013}? replacePassword)? ({this.serverVersion >= 80014}? retainCurrentPassword)?
            | AS_SYMBOL textStringHash ({this.serverVersion >= 80014}? retainCurrentPassword)?
            | {this.serverVersion >= 80018}? BY_SYMBOL RANDOM_SYMBOL PASSWORD_SYMBOL retainCurrentPassword?
        )?
    )
...
createUserStatement:
    CREATE_SYMBOL USER_SYMBOL ({this.serverVersion >= 50706}? ifNotExists)? createUserList defaultRoleClause createUserTail
;

createUserTail:
    {this.serverVersion >= 50706}? requireClause? connectOptions? accountLockPasswordExpireOptions* ({this.serverVersion >= 80021}? userAttributes)?
    | /* empty */
;
...
accountLockPasswordExpireOptions:
    ...
    | {this.serverVersion >= 80019}? FAILED_LOGIN_ATTEMPTS_SYMBOL real_ulong_number
    | {this.serverVersion >= 80019}? PASSWORD_LOCK_TIME_SYMBOL (real_ulong_number | UNBOUNDED_SYMBOL)
;
...
revokeStatement:
    REVOKE_SYMBOL ({this.serverVersion >= 80030}? ifExists)? (
    ...
    ) ({this.serverVersion >= 80030}? ignoreUnknownUser)?
;
...
roleOrPrivilege:
    {this.serverVersion > 80000}? (
        ...
        | roleIdentifierOrText AT_SIGN_SYMBOL (textOrIdentifier | SIMPLE_IDENTIFIER)
    )
...
optionValueNoOptionType:
    ...
    | AT_SIGN_SYMBOL AT_SIGN_SYMBOL setVarIdentType? lvalueVariable equal setExprOrDefault
    | NAMES_SYMBOL (
        equal expr
        | charsetName collate?
...
setExprOrDefault:
    ...
    | ALL_SYMBOL
    | BINARY_SYMBOL
    | {this.serverVersion >= 80000}? ROW_SYMBOL
    | {this.serverVersion >= 80000}? SYSTEM_SYMBOL
;
...
showReplicasStatement:
    SHOW_SYMBOL (SLAVE_SYMBOL HOSTS_SYMBOL | REPLICAS_SYMBOL)
;
...
showKeysStatement:
    SHOW_SYMBOL ({this.serverVersion >= 80000}? EXTENDED_SYMBOL)? (INDEX_SYMBOL | INDEXES_SYMBOL | KEYS_SYMBOL) fromOrIn tableRef inDb? whereClause?
;
...
showReplicaStatusStatement:
    SHOW_SYMBOL replica STATUS_SYMBOL ({this.serverVersion < 50706}? NONBLOCKING_SYMBOL)? channel?
;
...
simpleExpr:
    ...
    | {this.serverVersion >= 80022}? CAST_SYMBOL OPEN_PAR_SYMBOL expr AT_SYMBOL TIME_SYMBOL ZONE_SYMBOL
         INTERVAL_SYMBOL? textStringLiteral AS_SYMBOL DATETIME_SYMBOL typeDatetimePrecision CLOSE_PAR_SYMBOL    # simpleExprCastTime
...
sumExpr:
    name = AVG_SYMBOL OPEN_PAR_SYMBOL DISTINCT_SYMBOL? inSumExpr CLOSE_PAR_SYMBOL ({this.serverVersion >= 80000}? windowingClause)?
    | name = (BIT_AND_SYMBOL | BIT_OR_SYMBOL | BIT_XOR_SYMBOL) OPEN_PAR_SYMBOL inSumExpr CLOSE_PAR_SYMBOL ({this.serverVersion >= 80000}? windowingClause)?
    | {this.serverVersion >= 80000}? jsonFunction
    | {this.serverVersion >= 80024}? name = ST_COLLECT_SYMBOL OPEN_PAR_SYMBOL DISTINCT_SYMBOL? inSumExpr CLOSE_PAR_SYMBOL windowingClause?
    ...
    | name = (MIN_SYMBOL | MAX_SYMBOL) OPEN_PAR_SYMBOL DISTINCT_SYMBOL? inSumExpr CLOSE_PAR_SYMBOL ({this.serverVersion >= 80000}? windowingClause)?
    | name = (
        STD_SYMBOL
        | VARIANCE_SYMBOL
        | STDDEV_SAMP_SYMBOL
        | VAR_SAMP_SYMBOL
        | SUM_SYMBOL
    ) OPEN_PAR_SYMBOL inSumExpr CLOSE_PAR_SYMBOL ({this.serverVersion >= 80000}? windowingClause)?
    | name = SUM_SYMBOL OPEN_PAR_SYMBOL DISTINCT_SYMBOL inSumExpr CLOSE_PAR_SYMBOL ({this.serverVersion >= 80000}? windowingClause)?
...
runtimeFunctionCall:
    ...
    | {this.serverVersion >= 80021}? JSON_VALUE_SYMBOL OPEN_PAR_SYMBOL simpleExpr COMMA_SYMBOL textLiteral returningType? onEmptyOrError CLOSE_PAR_SYMBOL
...
geometryFunction:
    {this.serverVersion < 50706}? CONTAINS_SYMBOL OPEN_PAR_SYMBOL expr COMMA_SYMBOL expr CLOSE_PAR_SYMBOL
...
userVariable:
    AT_SIGN_SYMBOL userVariableIdentifier
;

userVariableIdentifier:
    textOrIdentifier | SIMPLE_IDENTIFIER
;
...
rvalueSystemOrUserVariable:
    userVariable
    | AT_SIGN_SYMBOL AT_SIGN_SYMBOL rvalueSystemVariableType? rvalueSystemVariable
;
...
createUser:
    user (
        identification ({this.serverVersion >= 80027}? createUserWithMfa)?
        | identifiedWithPlugin  ({this.serverVersion >= 80027}? initialAuth)?
        | {this.serverVersion >= 80027}? createUserWithMfa
    )?
;
...
identification:
    identifiedByPassword
    | {this.serverVersion >= 80018}? identifiedByRandomPassword
    | identifiedWithPlugin
    | identifiedWithPluginAsAuth
    | {this.serverVersion >= 50706}? identifiedWithPluginByPassword
    | {this.serverVersion >= 80018}? identifiedWithPluginByRandomPassword
;

identifiedByPassword:
    IDENTIFIED_SYMBOL BY_SYMBOL ({this.serverVersion < 80011}? PASSWORD_SYMBOL)? textStringLiteral
;
...
userIdentifierOrText:
    textOrIdentifier (AT_SIGN_SYMBOL (textOrIdentifier | SIMPLE_IDENTIFIER))?
;
...
optionType:
    {this.serverVersion >= 80000}? PERSIST_SYMBOL
    | {this.serverVersion >= 80000}? PERSIST_ONLY_SYMBOL
    | GLOBAL_SYMBOL
    | LOCAL_SYMBOL
    | SESSION_SYMBOL
;
