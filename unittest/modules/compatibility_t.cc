...
void check_sql_syntax(
  ...
  mysqlshdk::yacc::Parser parser{version};

  if (const auto issues = parser.check_syntax(stmt); !issues.empty()) {
    throw issues.front();
  }
}
#endif
...
TEST_F(Compatibility_test, lock_account) {
  ...
  const auto EXPECT = [](std::string_view statement,
    ...
    EXPECT_EQ(expected, lock_account(statement));
    EXPECT_NO_THROW(check_sql_syntax(expected));
  };

  EXPECT("CREATE USER u@h", "CREATE USER u@h ACCOUNT LOCK");

  EXPECT("CREATE USER u@h ACCOUNT LOCK", "CREATE USER u@h ACCOUNT LOCK");
  EXPECT("CREATE USER u@h account UNLOCK", "CREATE USER u@h account LOCK");

  EXPECT("CREATE USER u@h REQUIRE NONE account LOCK",
         "CREATE USER u@h REQUIRE NONE account LOCK");
  EXPECT("CREATE USER u@h REQUIRE NONE ACCOUNT UNLOCK",
         "CREATE USER u@h REQUIRE NONE ACCOUNT LOCK");

  EXPECT("CREATE USER u@h COMMENT 'a'",
         "CREATE USER u@h ACCOUNT LOCK COMMENT 'a'");
  EXPECT("CREATE USER u@h account LOCK COMMENT 'a'",
         "CREATE USER u@h account LOCK COMMENT 'a'");
  EXPECT("CREATE USER u@h ACCOUNT UNLOCK COMMENT 'a'",
         "CREATE USER u@h ACCOUNT LOCK COMMENT 'a'");

  EXPECT("CREATE USER u@h REQUIRE NONE COMMENT 'a'",
         "CREATE USER u@h REQUIRE NONE ACCOUNT LOCK COMMENT 'a'");
  EXPECT("CREATE USER u@h REQUIRE NONE ACCOUNT LOCK COMMENT 'a'",
         "CREATE USER u@h REQUIRE NONE ACCOUNT LOCK COMMENT 'a'");
  EXPECT("CREATE USER u@h REQUIRE NONE account UNLOCK COMMENT 'a'",
         "CREATE USER u@h REQUIRE NONE account LOCK COMMENT 'a'");

  EXPECT(R"(CREATE USER u@h REQUIRE CIPHER 'cipher')",
         R"(CREATE USER u@h REQUIRE CIPHER 'cipher' ACCOUNT LOCK)");
  EXPECT(
      R"(CREATE USER u@h REQUIRE CIPHER 'cipher' ATTRIBUTE '{"a", "b"}')",
      R"(CREATE USER u@h REQUIRE CIPHER 'cipher' ACCOUNT LOCK ATTRIBUTE '{"a", "b"}')");
  EXPECT(
      R"(CREATE USER u@h REQUIRE CIPHER 'cipher' ACCOUNT LOCK ATTRIBUTE '{"a", "b"}')",
      R"(CREATE USER u@h REQUIRE CIPHER 'cipher' ACCOUNT LOCK ATTRIBUTE '{"a", "b"}')");
  EXPECT(
      R"(CREATE USER u@h REQUIRE CIPHER 'cipher' ACCOUNT UNLOCK ATTRIBUTE '{"a", "b"}')",
      R"(CREATE USER u@h REQUIRE CIPHER 'cipher' ACCOUNT LOCK ATTRIBUTE '{"a", "b"}')");

  EXPECT(R"(CREATE USER u@h WITH MAX_QUERIES_PER_HOUR 123)",
         R"(CREATE USER u@h WITH MAX_QUERIES_PER_HOUR 123 ACCOUNT LOCK)");
  EXPECT(
      R"(CREATE USER u@h WITH MAX_QUERIES_PER_HOUR 123 COMMENT 'a')",
      R"(CREATE USER u@h WITH MAX_QUERIES_PER_HOUR 123 ACCOUNT LOCK COMMENT 'a')");
  EXPECT(
      R"(CREATE USER u@h WITH MAX_QUERIES_PER_HOUR 123 ACCOUNT LOCK COMMENT 'a')",
      R"(CREATE USER u@h WITH MAX_QUERIES_PER_HOUR 123 ACCOUNT LOCK COMMENT 'a')");
  EXPECT(
      R"(CREATE USER u@h WITH MAX_QUERIES_PER_HOUR 123 ACCOUNT UNLOCK COMMENT 'a')",
      R"(CREATE USER u@h WITH MAX_QUERIES_PER_HOUR 123 ACCOUNT LOCK COMMENT 'a')");

  EXPECT(R"(CREATE USER u@h PASSWORD EXPIRE INTERVAL 5 DAY)",
         R"(CREATE USER u@h PASSWORD EXPIRE INTERVAL 5 DAY ACCOUNT LOCK)");
  EXPECT(
      R"(CREATE USER u@h PASSWORD EXPIRE INTERVAL 5 DAY ATTRIBUTE '{"a", "b"}')",
      R"(CREATE USER u@h PASSWORD EXPIRE INTERVAL 5 DAY ACCOUNT LOCK ATTRIBUTE '{"a", "b"}')");
  EXPECT(
      R"(CREATE USER u@h PASSWORD EXPIRE INTERVAL 5 DAY ACCOUNT LOCK ATTRIBUTE '{"a", "b"}')",
      R"(CREATE USER u@h PASSWORD EXPIRE INTERVAL 5 DAY ACCOUNT LOCK ATTRIBUTE '{"a", "b"}')");
  EXPECT(
      R"(CREATE USER u@h PASSWORD EXPIRE INTERVAL 5 DAY ACCOUNT UNLOCK ATTRIBUTE '{"a", "b"}')",
      R"(CREATE USER u@h PASSWORD EXPIRE INTERVAL 5 DAY ACCOUNT LOCK ATTRIBUTE '{"a", "b"}')");
}

TEST_F(Compatibility_test, replace_authentication_plugin) {
  EXPECT_THROW(replace_authentication_plugin("CREATE SCHEMA s", "plugin"),
               std::runtime_error);
  ...
  const auto EXPECT = [](std::string_view statement,
    ...
    EXPECT_EQ(expected, replace_authentication_plugin(statement, "plugin"));
    EXPECT_NO_THROW(check_sql_syntax(expected));
  };

  EXPECT("CREATE USER u@h", "CREATE USER u@h");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin'",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH \"plugin\"",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
  ...
  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' REQUIRE NONE",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' BY 'auth_string'",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' BY 'auth_string' REQUIRE NONE",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' BY RANDOM PASSWORD",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' BY RANDOM PASSWORD REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' AS 'auth_string'",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' AS 'auth_string' REQUIRE NONE",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' INITIAL AUTHENTICATION IDENTIFIED BY RANDOM PASSWORD)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' INITIAL AUTHENTICATION IDENTIFIED BY RANDOM PASSWORD REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' INITIAL AUTHENTICATION IDENTIFIED BY 'auth_string')",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' INITIAL AUTHENTICATION IDENTIFIED BY 'auth_string' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' INITIAL AUTHENTICATION IDENTIFIED WITH 'plugin' AS 'auth_string')",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' INITIAL AUTHENTICATION IDENTIFIED WITH 'plugin' AS 'auth_string' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' INITIAL AUTHENTICATION IDENTIFIED WITH 'plugin-1' AS 'auth_string')",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin' INITIAL AUTHENTICATION IDENTIFIED WITH 'plugin-1' AS 'auth_string' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin-1' INITIAL AUTHENTICATION IDENTIFIED WITH 'plugin' AS 'auth_string')",
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin-1' INITIAL AUTHENTICATION IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin-1' INITIAL AUTHENTICATION IDENTIFIED WITH 'plugin' AS 'auth_string' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin-1' INITIAL AUTHENTICATION IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' AND IDENTIFIED WITH 'plugin'",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' AND IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin-1' AND IDENTIFIED WITH 'plugin'",
      R"(CREATE USER u@h IDENTIFIED WITH 'plugin-1' AND IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' AND IDENTIFIED WITH 'plugin-1'",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' AND IDENTIFIED WITH 'plugin-1')");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' REQUIRE CIPHER 'cipher'",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE CIPHER 'cipher')");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' WITH MAX_QUERIES_PER_HOUR 123",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' WITH MAX_QUERIES_PER_HOUR 123)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'plugin' PASSWORD EXPIRE INTERVAL 5 DAY",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' PASSWORD EXPIRE INTERVAL 5 DAY)");
}

TEST_F(Compatibility_test, replace_empty_passwords) {
  EXPECT_THROW(replace_empty_passwords("CREATE SCHEMA s"), std::runtime_error);

  const auto EXPECT = [](std::string_view statement, std::string_view expected,
                         const Version &version =
                             mysqlshdk::utils::k_shell_version) {
    SCOPED_TRACE(statement);

    EXPECT_EQ(expected, replace_empty_passwords(statement));
    EXPECT_NO_THROW(check_sql_syntax(expected, version));
  };
  ...
  EXPECT(
      ...
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      "CREATE USER u@h IDENTIFIED BY '' REQUIRE NONE",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT("CREATE USER u@h IDENTIFIED BY 'pass'",
         R"(CREATE USER u@h IDENTIFIED BY 'pass')");
  EXPECT("CREATE USER u@h IDENTIFIED BY 'pass' REQUIRE NONE",
         R"(CREATE USER u@h IDENTIFIED BY 'pass' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED BY "")",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED BY "" REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(R"(CREATE USER u@h IDENTIFIED BY "pass")",
         R"(CREATE USER u@h IDENTIFIED BY "pass")");
  EXPECT(R"(CREATE USER u@h IDENTIFIED BY "pass" REQUIRE NONE)",
         R"(CREATE USER u@h IDENTIFIED BY "pass" REQUIRE NONE)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED BY PASSWORD ''",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      "CREATE USER u@h IDENTIFIED BY PASSWORD '' REQUIRE NONE",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT("CREATE USER u@h IDENTIFIED BY PASSWORD 'pass'",
         R"(CREATE USER u@h IDENTIFIED BY PASSWORD 'pass')", Version(5, 7, 40));
  EXPECT("CREATE USER u@h IDENTIFIED BY PASSWORD 'pass' REQUIRE NONE",
         R"(CREATE USER u@h IDENTIFIED BY PASSWORD 'pass' REQUIRE NONE)",
         Version(5, 7, 40));

  EXPECT("CREATE USER u@h IDENTIFIED BY RANDOM PASSWORD",
         R"(CREATE USER u@h IDENTIFIED BY RANDOM PASSWORD)");
  EXPECT("CREATE USER u@h IDENTIFIED BY RANDOM PASSWORD REQUIRE NONE",
         R"(CREATE USER u@h IDENTIFIED BY RANDOM PASSWORD REQUIRE NONE)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'sha256_password'",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'sha256_password' REQUIRE NONE",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY ''",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      "CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY '' REQUIRE NONE",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT("CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY 'pass'",
         R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY 'pass')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY 'pass' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY 'pass' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY RANDOM PASSWORD)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY RANDOM PASSWORD)");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY RANDOM PASSWORD REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY RANDOM PASSWORD REQUIRE NONE)");

  EXPECT("CREATE USER u@h IDENTIFIED WITH 'sha256_password' AS ''",
         R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' AS '')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' AS '' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' AS '' REQUIRE NONE)");

  EXPECT("CREATE USER u@h IDENTIFIED WITH 'sha256_password' AS 'hash'",
         R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' AS 'hash')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' AS 'hash' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' AS 'hash' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY RANDOM PASSWORD)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY RANDOM PASSWORD)");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY RANDOM PASSWORD REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY RANDOM PASSWORD REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY '')",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY '' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY 'pass')",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY 'pass')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY 'pass' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED BY 'pass' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED WITH 'sha256_password' AS '')",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED WITH 'sha256_password' AS '')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED WITH 'sha256_password' AS '' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED WITH 'sha256_password' AS '' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED WITH 'sha256_password' AS 'hash')",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED WITH 'sha256_password' AS 'hash')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED WITH 'sha256_password' AS 'hash' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' INITIAL AUTHENTICATION IDENTIFIED WITH 'sha256_password' AS 'hash' REQUIRE NONE)");

  EXPECT(
      "CREATE USER u@h IDENTIFIED BY '' AND IDENTIFIED BY PASSWORD ''",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' AND IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED BY '' AND IDENTIFIED BY PASSWORD '' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' AND IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' AND IDENTIFIED WITH 'sha256_password' BY 'pass')",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' AND IDENTIFIED WITH 'sha256_password' BY 'pass')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' AND IDENTIFIED WITH 'sha256_password' BY 'pass' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' AND IDENTIFIED WITH 'sha256_password' BY 'pass' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED BY 'pass' AND IDENTIFIED WITH 'sha256_password')",
      R"(CREATE USER u@h IDENTIFIED BY 'pass' AND IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED BY 'pass' AND IDENTIFIED WITH 'sha256_password' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED BY 'pass' AND IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED BY 'pass' AND IDENTIFIED WITH 'sha256_password' BY 'pass')",
      R"(CREATE USER u@h IDENTIFIED BY 'pass' AND IDENTIFIED WITH 'sha256_password' BY 'pass')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED BY 'pass' AND IDENTIFIED WITH 'sha256_password' BY 'pass' REQUIRE NONE)",
      R"(CREATE USER u@h IDENTIFIED BY 'pass' AND IDENTIFIED WITH 'sha256_password' BY 'pass' REQUIRE NONE)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' REQUIRE CIPHER 'cipher')",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' REQUIRE CIPHER 'cipher')");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY 'pass' REQUIRE CIPHER 'cipher')",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY 'pass' REQUIRE CIPHER 'cipher')");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' WITH MAX_QUERIES_PER_HOUR 123)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' WITH MAX_QUERIES_PER_HOUR 123)");
  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY 'pass' WITH MAX_QUERIES_PER_HOUR 123)",
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' BY 'pass' WITH MAX_QUERIES_PER_HOUR 123)");

  EXPECT(
      R"(CREATE USER u@h IDENTIFIED WITH 'sha256_password' PASSWORD EXPIRE INTERVAL 5 DAY)",
      R"(CREATE USER u@h IDENTIFIED WITH 'caching_sha2_password' AS '$A$005$THISISACOMBINATIONOFINVALIDSALTANDPASSWORDTHATMUSTNEVERBRBEUSED' PASSWORD EXPIRE INTERVAL 5 DAY)");
  EXPECT(
