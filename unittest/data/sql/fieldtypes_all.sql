...
-- MySQL accepts the parenthesized MULTIPOINT form and nested GEOMETRYCOLLECTION;
-- MariaDB's ST_GeomFromText() rejects both (returns NULL). Set those two values
-- the MySQL way first (runs on both engines), then override them on MariaDB only
-- via /*M! ... */ executable comments (ignored by MySQL): a flat MULTIPOINT
-- (identical geometry/WKB) and a non-nested GEOMETRYCOLLECTION.
SET @mp = ST_GeomFromText('MULTIPOINT((4541233412.134314 56343456.56345634),(53425234.23453452 56435634.23454523),(-43231234.45 -552342345.534))');
/*M!100000 SET @mp = ST_GeomFromText('MULTIPOINT(4541233412.134314 56343456.56345634,53425234.23453452 56435634.23454523,-43231234.45 -552342345.534)') */;
SET @gc = ST_GeomFromText('GEOMETRYCOLLECTION(GEOMETRYCOLLECTION(LINESTRING(0 0, 1 1)),LINESTRING(2 2, 3 3))');
/*M!100000 SET @gc = ST_GeomFromText('GEOMETRYCOLLECTION(LINESTRING(0 0, 1 1),LINESTRING(2 2, 3 3))') */;
...
/*!90000 CREATE TABLE t_vector (v VECTOR(8)) */;
/*!90000 INSERT INTO t_vector VALUES (UNHEX("8679613f")), (UNHEX("71b06a3f")), (UNHEX("95afc03e")), (UNHEX("c620123d")), (UNHEX("b8d58c3d")) */;
-- MariaDB has VECTOR since 11.7, but VECTOR(N) is a fixed N-dimension type, so
-- the single-element (4-byte) values above need VECTOR(1). The binary format and
-- HEX output match MySQL, so the test expectation is identical. mysqlsh strips
-- /*!90000*/ against MariaDB (it reads the 5.5.5 handshake-compat version), hence
-- the MariaDB-only /*M! ... */ executable comments below.
/*M!110700 CREATE TABLE t_vector (v VECTOR(1)) */;
/*M!110700 INSERT INTO t_vector VALUES (UNHEX("8679613f")), (UNHEX("71b06a3f")), (UNHEX("95afc03e")), (UNHEX("c620123d")), (UNHEX("b8d58c3d")) */;
