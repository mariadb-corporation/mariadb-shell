...
if instance_supports_libraries:
    ...
    EXPECT_STDOUT_CONTAINS("1 schemas will be dumped and within them 16 tables, 7 views, 1 event, 1 library, 8 routines, 6 triggers.")
...
# test
EXPECT_SUCCESS(__sandbox_uri2, { "showProgress": False }, schemas=[tested_schema])
...
for account in ["mysql.infoschema", "mysql.session", "mysql.sys", "ociadmin", "ocidbm", "ocirpl"]:
    ...
    setup_db(account)
    WIPE_OUTPUT()
    EXPECT_FAIL("Error: Shell Error (52004)", "Compatibility issues were found", __sandbox_uri2, { "dryRun": True, "showProgress": False }, schemas = [ schema_name ])
