...
"GRANT APPLICATION_PASSWORD_ADMIN,CONNECTION_ADMIN,REPLICATION_APPLIER,ROLE_ADMIN,RESOURCE_GROUP_ADMIN,RESOURCE_GROUP_USER,XA_RECOVER_ADMIN ON *.* TO `administrator`@`%` WITH GRANT OPTION",
...
mds_user_grants = [
    ...
    "GRANT APPLICATION_PASSWORD_ADMIN,CONNECTION_ADMIN,REPLICATION_APPLIER,ROLE_ADMIN,RESOURCE_GROUP_ADMIN,RESOURCE_GROUP_USER,XA_RECOVER_ADMIN ON *.* TO {} WITH GRANT OPTION",
...
# excluded role
excluded_role = "sample-excluded-role"
session2.run_sql(f"CREATE ROLE IF NOT EXISTS '{excluded_role}'")
session2.run_sql("create user myuser4@'%' IDENTIFIED BY 'password'")
session2.run_sql(f"grant '{excluded_role}' to myuser4@'%'")
...
# dump with root
util.dump_instance(os.path.join(outdir, "dump_root"), {"compatibility":["strip_restricted_grants", "strip_definers"], "excludeUsers": [excluded_role], "ocimds":True})
...
EXPECT_STDOUT_CONTAINS(f"WARNING: User 'myuser4'@'%' has a grant statement on a role `{excluded_role}`@`%` which is not included in the dump (")
# BUG#38264847 - warning has been downgraded to a note if role is an MHS role which got automatically excluded
EXPECT_STDOUT_CONTAINS("NOTE: User 'admin'@'%' has a grant statement on a role `administrator`@`%` which is not included in the dump (")
...
GRANT APPLICATION_PASSWORD_ADMIN,CONNECTION_ADMIN,REPLICATION_APPLIER,ROLE_ADMIN,XA_RECOVER_ADMIN ON *.* TO `myuser2`@`%`
...
GRANT APPLICATION_PASSWORD_ADMIN,CONNECTION_ADMIN,REPLICATION_APPLIER,ROLE_ADMIN,XA_RECOVER_ADMIN ON *.* TO `myuser3`@`%` WITH GRANT OPTION
