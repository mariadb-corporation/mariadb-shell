...
testutil.call_mysqlsh(["--credential-store-helper=plaintext", "--ssh", test_ssh_uri_nopwd, MYSQL_OVER_SSH_URI, "--ssh-config-file", config_file], "Cancel", ["MARIADB_SHELL_TERM_COLOR_MODE=nocolor"], os.path.join(__bin_dir, "mysqlsh"))
...
EXPECT_THROWS(lambda: shell.connect(conn), "Error opening MySQL session through SSH tunnel: Lost connection to MySQL server at '")
conn["uri"] = ""
