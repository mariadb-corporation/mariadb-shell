...
MACRO(GET_MYSQL_VERSION)
  IF(MYSQL_SOURCE_DIR)
    SET(DB_SRC_DIR "${MYSQL_SOURCE_DIR}")
    SET(VERSION_FILE_NAME "MYSQL_VERSION")
  ELSEIF(MARIADB_SOURCE_DIR)
    SET(DB_SRC_DIR "${MARIADB_SOURCE_DIR}")
    SET(VERSION_FILE_NAME "VERSION")
  ENDIF()

  IF(DB_SRC_DIR)
    # 1. Standardize path and find the version file
    FILE(TO_CMAKE_PATH "${DB_SRC_DIR}" DB_SRC_ROOT)
    SET(VERSION_FILE_PATH "${DB_SRC_ROOT}/${VERSION_FILE_NAME}")

    # 2. Extract version components
    MYSQL_GET_CONFIG_VALUE("${VERSION_FILE_PATH}" "MYSQL_VERSION_MAJOR" MYSQL_MAJOR_VERSION)
    MYSQL_GET_CONFIG_VALUE("${VERSION_FILE_PATH}" "MYSQL_VERSION_MINOR" MYSQL_MINOR_VERSION)
    MYSQL_GET_CONFIG_VALUE("${VERSION_FILE_PATH}" "MYSQL_VERSION_PATCH" MYSQL_PATCH_VERSION)
    MYSQL_GET_CONFIG_VALUE("${VERSION_FILE_PATH}" "MYSQL_VERSION_EXTRA" MYSQL_EXTRA_VERSION)

    IF(NOT DEFINED MYSQL_MAJOR_VERSION OR
      NOT DEFINED MYSQL_MINOR_VERSION OR
      NOT DEFINED MYSQL_PATCH_VERSION)
      MESSAGE(FATAL_ERROR "${VERSION_FILE_NAME} file for server cannot be parsed.")
    ENDIF()
