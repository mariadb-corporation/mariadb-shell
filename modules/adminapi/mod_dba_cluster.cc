...
@attention Setting this option to a value other than the default may have unwanted
...
partitions of the target Cluster, thus return different data; and different Routers
may also have different behavior (i.e. some provide only read traffic while others
read and write traffic). Note that writes on a partition of the target Cluster with
no quorum will block until quorum is restored.
