...
std::vector<Upgrade_issue> Invalid_engine_foreign_key_check::run(
  ...
  return context.execute_simple(
      this, [this, &context](
        ...
        // These tables don't have indexes and in large DBs JOINs can take
        // literally hours with a few thousand FKs, so we do the JOINs here.
        // Temporary tables would solve the problem in an easier way, but their
        // size is limited and it's too easy to fill them up.
        ...
        while ((row = result->fetch_one())) {
          ...
          // FK sreferences may be stored in different case sensitivity
          // depending on the OS and the MySQL configuration, so we need to
          // search for table references disabling case sensitivity

          auto table_name =
              internal::decode_escaped_table_name(row->get_string(1));
          auto table = cache->get_table(table_name, false);

          auto ref_name =
              internal::decode_escaped_table_name(row->get_string(2));
        ...
        // Early exit, if there are not even FKs that met the criteria being
        // searched, we are done.
        ...
        while ((row = result->fetch_one())) {
          ...
          if (!fk->second.ref_columns.empty())
            fk->second.ref_columns.append(",");
